    ...
    for i in range(len(present_blocks) - 1):
        ...
        if former_pos > latter_pos:
            declaration_errors.append(
                (
                    {former_key: "..."},
                    "{} should be declared before {}".format(former_key, latter_key),
                )
            )
    ...
    for i in range(len(options) - 1):
        ...
        if former > latter:
            declaration_errors.append(
                (
                    {former: "..."},
                    "{} should be declared after {}".format(former, latter),
                )
            )
...
def get_ordered_play_from_file(play, file):
    ...
    while (
        not content[play_end_line_no].startswith("-")
        and play_end_line_no < len(content) - 1
    ):
        play_end_line_no += 1
...
class HostsDeclaration(AnsibleLintRule):
    ...
    description = (
        "Hosts declaration should use the following order: `hosts:` declaration, host options in alphabetical"
