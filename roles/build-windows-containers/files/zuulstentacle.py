...
class ZuulJenkinsConnector:
    def __init__(self, repository_provider, jenkins_manager, jenkins_job,\
            gerrit_server, docker_registry_ip, docker_registry_port):
        self.repository_provider = repository_provider
        self.jenkins_manager = jenkins_manager
        self.jenkins_job = jenkins_job
        self.gerrit_server = gerrit_server
        self.docker_registry_ip = docker_registry_ip
        self.docker_registry_port = docker_registry_port
    ...
    def set_repositories_root_dir_as_current_dir(self):
        self.repo_directories_prefix = os.path.join("src", self.gerrit_server)
        os.chdir(self.repo_directories_prefix)
    ...
    def start_jenkins_job_and_wait_for_result(self):
        ...
        print(("Starting Jenkins job\n" +
            "    job name: \"{}\"\n" +
            "Docker registry IP: {}" .format(self.docker_registry_ip) +
            "Docker registry port {}" .format(self.docker_registry_port) +
...
class RepositoryProvider:
    def __init__(self, archive_base_name, http_server_ip, http_server_port,floating_ip):
...
class ConfigurationManager:
    ...
    def setup_argument_parser(self):
        ...
        self.argument_parser.add_argument(
            "--docker_registry_ip",
            dest="docker_registry_ip",
            default=0,
            help="Temporary docker registry ip for test purpose only")
        self.argument_parser.add_argument(
            "--docker_registry_port",
            dest="docker_registry_port",
            default=0,
            help="Temporary docker registry port for test purpose only")
...
def CreateZuulJenkinsConnector(configuration):
    ...
    zuuls_tentacle = ZuulJenkinsConnector(
        ...
        configuration.docker_registry_ip,
        configuration.docker_registry_port)
