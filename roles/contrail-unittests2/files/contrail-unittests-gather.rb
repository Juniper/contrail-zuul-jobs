...
json.each_pair { |module_name, module_data|
    ...
    next if skip
    STDERR.puts "contrail-unittest-gather.rb: SCons targets to run: #{module_data['scons_test_targets']}\n"

    @tests += module_data["scons_test_targets"]
    module_data["misc_test_targets"].each { |m|
        @tests += json[m]["scons_test_targets"]
	STDERR.puts "contrail-unittest-gather.rb: SCons misc targets: #{json[m]['scons_test_targets']}\n"
    }
}
